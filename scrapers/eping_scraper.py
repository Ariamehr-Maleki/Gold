...
class EPingScraper:
    ...
    # ... (All internal methods like _get_firefox_options, start_browser, _filter_by_hs_code, etc. remain unchanged) ...
    ...
    def _filter_by_hs_code(self, hs_code_prefix):
        """Robust HS-code selection for vue-treeselect component."""
        try:
            ...
            self.driver.execute_script("arguments[0].textContent = arguments[1]; arguments[0].dispatchEvent(new Event('input', {bubbles: true}));", hs_input, hs_code_prefix)
            time.sleep(1)
            ...
            logging.info("Successfully filtered by HS code.")
            time.sleep(5)
            return True
        except Exception as e:
