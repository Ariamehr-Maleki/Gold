...
# ─── Data fetchers (exact logic from spec) ──────────────────────────────────
...
def fetch_gold_data() -> dict:
    try:
        ...
        print(f"❌ خطای شبکه در دریافت طلا: {e}")
        return {}
...
def fetch_usd_data() -> float:
    ...
    try:
        ...
        elif "tehran_naghdi_sell" in data:
            return float(data["tehran_naghdi_sell"]["value"])
        elif "harat_naghdi_sell" in data:
            return float(data["harat_naghdi_sell"]["value"])
        else:
            print("⚠️ کلید قیمت دلار یافت نشد.")
            return 0
    except requests.exceptions.RequestException as e:
        print(f"❌ خطای شبکه در دریافت دلار: {e}")
        return 0


# ─── Price calculation (exact logic from spec) ──────────────────────────────

def calculate_item(code: str, market_price: float, ounce: float, usd: float, gold18k: float):
    """Returns (real_value, bubble_absolute, bubble_percent) or (None, None, None)."""
