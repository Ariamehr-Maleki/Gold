# macmap_scraper.py (Updated for new DOM)
...
class MacMapScraper(TradeSpider):
    """
    A dedicated scraper for macmap.org, updated to match the new DOM structure.
    It scrapes customs tariffs, interactive trade remedies, and regulatory requirements.
    """
    ...
    def scrape_market_access(self, config):
        ...
        # --- FIRST LOAD (warm-up load) ---
        logging.info("Loading MacMap page (first pass)...")
        if not self.goto(url):
            return None
        time.sleep(2)
        
        # wait for partial DOM to load
        try:
            self.wait.until(lambda d: d.execute_script("return document.readyState") == "complete")
        except TimeoutException:
            logging.warning("First page load not fully complete, continuing to reload...")

        # --- SECOND LOAD (actual scrape) ---
        logging.info("Reloading page for stable DOM...")
        self.driver.get(url)
        time.sleep(3)
        try:
            self.wait.until(lambda d: d.execute_script("return document.readyState") == "complete")
            logging.info("Page fully loaded on second attempt.")
        except TimeoutException:
            logging.warning("Second load timed out; proceeding anyway.")
        ...
        try:
            ...
            logging.error("Timed out waiting for overview content after reload.")
...
if __name__ == '__main__':
    ...
    CONFIG = {
        ...
        "hs_code": "847130",
        "your_country": "China", "your_country_id": "156",
